...
def propagate(elev: np.ndarray, cell_m: float, p: ScenarioParams) -> SimResult:
    ...
    rain_add = rain_rate * dt * rain_jitter
    ...
    n_cells = max(1, (c1 - c0) * (r1 - r0))
    depth0 = min(float(p.breach_depth_m), 200.0) * SEVERITY_MULT[p.breach_severity]
    h[r0:r1, c0:c1] = depth0
    arrival[r0:r1, c0:c1] = 0.0
    maxdepth[r0:r1, c0:c1] = depth0
    volume_in = float(depth0 * n_cells * area)
    ...
    inflow_depth_per_step = inflow_rate * dt / (n_cells * area)
    ...
    for step in range(1, steps + 1):
        ...
        if t_min * 60.0 <= inflow_total_s:
            h[r0:r1, c0:c1] += inflow_depth_per_step
        ...
        # Manning-analogue speed per face (donor-cell depth)
        h23 = np.power(np.maximum(h, 0.0), 2.0 / 3.0)
        coef = h23 / n_manning * dt * dx  # flux volume per unit slope^0.5
...
def run_scenario(elev: np.ndarray, cell_m: float, p: ScenarioParams) -> SimResult:
    """Seam for a future rigorous solver: same inputs, same outputs."""
    return propagate(elev, cell_m, p)
