...
IDs (d1..d131 namespace, 50 curated entries) are stable canonical keys shared
with terrain assets.

Every entry also carries the curated public facts (type, river, capacity,
construction year) used by the admin dam profile and the condition assessment.
These are CONTEXT data: dam material/type is never treated as a safety verdict.
"""
...
# Dams registered through the app at runtime (admin/analyst onboarding) live in
# a small JSON file beside the other local stores. They become first-class
# registry entries: profiles, condition, terrain capture and (with a DEM) all
# simulation paths work for them.
from pathlib import Path as _Path
import json as _json

REGISTERED_PATH = _Path(__file__).resolve().parent.parent.parent / "data" / "registered_dams.json"
...
def _load_registered() -> list[dict]:
    if not REGISTERED_PATH.exists():
        return []
    try:
        return _json.loads(REGISTERED_PATH.read_text(encoding="utf-8"))
    except Exception:
        return []


def register_dam(entry: dict) -> dict:
    """Append a runtime-registered dam (id must be 'r'-prefixed, coords required)."""
    dams = _load_registered()
    dams.append(entry)
    REGISTERED_PATH.parent.mkdir(parents=True, exist_ok=True)
    REGISTERED_PATH.write_text(_json.dumps(dams, indent=2, ensure_ascii=False), encoding="utf-8")
    return entry


def all_dams() -> list[dict]:
    """Curated registry + runtime registrations (curated wins on id clash)."""
    return DAMS + [d for d in _load_registered() if d.get("id") not in _BY_ID]


def get_dam(dam_id: str) -> dict | None:
    """Canonical record (curated first, then runtime registrations) or None."""
    if dam_id in _BY_ID:
        return _BY_ID[dam_id]
    for d in _load_registered():
        if d.get("id") == dam_id:
            return d
    return None


def terrain_ready(dam_id: str) -> bool:
    """True when a simulation-grade DEM for this dam exists in any terrain root."""
    from app.sandbox.terrain import locate_dem

    try:
        return locate_dem(dam_id) is not None
    except Exception:
        return False
